...
from utils import (get_children, get_teachers, get_school_analytics, download_csv,
...
async def parent_page(request: Request, family: str = None, action: str = None):
    logging.info(f"fetching all students with family name: {family}")
    children_data = await get_children()
...
async def manager_page(request: Request, subject: str = None, action: str = None):
    children_data = await get_children()
    teachers_data = await get_teachers()
    unregistered_students = get_unregistered_students(children_data)
    students_by_subjects, teachers_data, num_of_students = handle_manager_view(children=children_data,
